// MergeRelocate.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void Usage()
{
    cout << "Usage:" << endl << "\t" << "mergeRelocate relocate.prg mon1.prg mon2.prg outputfile.prg" << endl;
}

unsigned long long  StreamLength(ifstream& s)
{
    s.seekg(0, ios::end);
    const unsigned long long length = s.tellg();
    s.seekg(0, ios::beg);
    return length;
}
...
{
    // Little Endian
    const int n = static_cast<int>(static_cast<unsigned int>(buffer[offset]) + static_cast<unsigned int>(buffer[offset + 1]) * 256);
    return n;
}
